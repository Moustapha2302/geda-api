...
public function index(Request $request, int $service)
{
    ...
    $status = $request->query('status'); // pending, accepted, rejected, cancelled
    $type = $request->query('type'); // internal, external
    $direction = $request->query('direction'); // sent, received, all

    $query = Transfer::query();

    // Filtrer par direction
    if ($direction === 'sent') {
        $query->where('from_service_id', $service);
    } elseif ($direction === 'received') {
        $query->where('to_service_id', $service);
    } else {
        // Par défaut : tous les transferts (envoyés et reçus)
        $query->where(function($q) use ($service) {
            $q->where('from_service_id', $service)
    ...
    $transfers = $query->with([
                        'document' => function($q) {
                            $q->select('id', 'title', 'reference', 'status', 'service_id');
                        },
                        'fromService:id,name',
                        'toService:id,name',
