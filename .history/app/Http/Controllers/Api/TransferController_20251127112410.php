...
public function index(Request $request, $service)
{
     ...
     */
    public function accept(Request $request, int $service, Transfer $transfer)
{
    // 1. Vérifier que le transfert appartient bien au service connecté
    if ($transfer->to_service_id != $service) {
        return response()->json([
            'success' => false,
            'message' => 'Ce transfert ne vous est pas destiné'
        ], 403);
    }
    ...
    // 3. Vérifier que le transfert n'a pas expiré
    ...
    // 4. Marquer le transfert comme accepté
    $transfer->update([
        'status' => 'accepted',
        'processed_at' => now()
    ]);

    // 5. Transférer le document vers le service destinataire
    $transfer->document->update([
        'service_id' => $transfer->to_service_id
    ]);

    // 6. (Optionnel) Créer une notification ou un log
    Log::info('Transfer accepted', [
        'transfer_id' => $transfer->id,
        'document_id' => $transfer->document_id,
        'from_service' => $transfer->from_service_id,
        'to_service' => $transfer->to_service_id,
        'accepted_by' => $request->user()->id
    ]);

    // 7. Retourner la réponse avec les relations chargées
    return response()->json([
        'success' => true,
        'message' => 'Transfert accepté avec succès',
        'data' => $transfer->load([
            'document:id,title,reference,status,service_id',
            'fromService:id,name',
            'toService:id,name',
