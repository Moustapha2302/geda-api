...
use App\Models\User;                   // alias déjà présent
...
class AuthController extends Controller
{
    public function login(Request $request)
    {
        ...
        $user = User::where('email', $request->email)->first();          // ← sans \
        if (!$user || !Hash::check($request->password, $user->password)) { // ← sans \
    ...
    {
$request->user()->tokens()->where('id', $request->user()->currentAccessToken()->id)->delete();        return response()->json(['success' => true, 'message' => 'Déconnexion réussie']);
    }
