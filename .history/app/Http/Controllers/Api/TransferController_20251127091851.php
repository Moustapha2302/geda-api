...
{
    // Filtrer par service (source ou destination)
    $transfers = Transfer::where(function ($q) use ($service) {
        $q->where('from_service_id', $service)
          ->orWhere('to_service_id', $service);
    })
    ->with(['document', 'fromService', 'toService', 'initiatedBy', 'processedBy'])
    ...
    return response()->json([
        'success' => true,
        'data'    => $transfers,
        'message' => 'Liste des transferts pour le service ' . $service
    ]);
}
